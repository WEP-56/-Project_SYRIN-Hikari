...
class WebSearchTool(Tool):
    ...
    def __init__(self, api_key: str | None = None, provider: str = "brave", max_results: int = 5):
        self.api_key = api_key or os.environ.get("BRAVE_API_KEY", "")
        self.provider = provider
        self.max_results = max_results
        
        # Auto-switch to duckduckgo if no API key for Brave
    ...
    async def _search_duckduckgo(self, query: str, count: int) -> str:
        try:
            # Run in executor because DDGS is synchronous (or use async version if available)
            # DDGS now supports context manager usage
            results = []
            with DDGS() as ddgs:
